...
### Step 4: plot for rank estimate measure and output rank summary
rank_estimate_plot <- function(outputFolder,rankfilepath) {
  ...
  rank_blank <- data.frame(cancertype=file,rss_suggested_rank=NA,rank=NA)
  ...
  for (i in 1:length(file)) {
    tryCatch({
        ...
        rank_blank[which(rank_blank$cancertype == filename),]$rss_suggested_rank <- rss_decrease
