...
CountMatBuild <- function(samplelist,upper,input_folder,genelist=NA){
  ...
  if (n_sample==1) {
    ...
    ccfBandCountsMat.random <- sample(ccfBandCountsMat,(rows-1),replace=TRUE,prob=ccfBandFractionMat)
    ccfBandFractionMat.random <- ccfBandCountsMat.random/sum(ccfBandCountsMat.random)
    
    } else {
      
    ccfBandFractionMat <- apply(ccfBandCountsMat,2,function(x) x/sum(x))
    #ccfBandCountsMat.random <- NMF::randomize(ccfBandCountsMat)
    ccfBandCountsMat.random <- apply(ccfBandCountsMat,2,function(x) sample(x,(rows-1),replace=TRUE,prob= x/sum(x)))
...
TypeMatrixBuild <- function(post_summary,input_folder,output,ccfupper=1,RankEstimateType="fraction"){
  ...
  for (i in 1:ntype){
    ...
    type <-  typelist[i]
    if (dir.exists(paste0(output,type,"/"))) next else {
...
nmf_sig_plot_type <- function(type,MatType="fraction",input_folder,output,rank_summary,seed,num_cores,nmfmethod){
     library(magrittr)
  
     type_path <- paste0(input_folder,type,"/")
     if (MatType=="fraction") {file_path <- paste0(input_folder,type,"/",dir(type_path)[grep("ccfFractionMatrix_",dir( type_path))])} 
     if (MatType=="count") {file_path <- paste0(input_folder,type,"/",dir(type_path)[grep("ccfCountsMatrix_",dir( type_path))])}
     ...
     # delete rows with all 0 and record row index
     index_p <- which(rowSums(ccfMat)>0)
     index_n <- which(!rowSums(ccfMat)>0)
     ccfMat <- ccfMat[which(rowSums(ccfMat)>0),]
     
     # run NMF
     res <- nmf(ccfMat,rank,.opt=paste0('p',num_cores),nrun=1000,seed=seed,method=nmfmethod)
     
     # add rows with all zero back 
     ...
     # add samplename to exposure 
     expo <- as.data.frame(t(expo)) %>% set_colnames(paste0("sig_",1:ncol(.))) %>% cbind(.,samplename) 
...
nmf_sig_all_plot <- function(input_folder,output,rank_summary,seed=123456,num_cores=4,nmfmethod="brunet") {
  library(gridExtra)
  library(ggplot2)
  library(grid)
  library(dplyr)
  ...
  for (i in 1:length(cancertype)) {
    ...
    nmf_result <- nmf_sig_plot_type(type,input_folder=input_folder,output=output,rank_summary=rank_summary,seed=seed,num_cores=num_cores,nmfmethod=nmfmetho)
    p_sig <- nmf_result[[1]]
...
## pre-step: set directory and Initialization
  ...
  Matrix_folder <- paste0("3.ccfMatrix_sample/")
...
## Combine signatures extracted from differernt cancer types, run Hierarchical cluster and plot (save hc_heatmap, consensus sig and plot to output folder)
  ...
  consensus_sig_eu <- hc_consensus(combine_sig,cluster=2,ccfMatrix=ccfMat[[3]],output=HC_folder,distance="euclidean")
  consensus_sig_correlation <- hc_consensus(combine_sig,cluster=2,ccfMatrix=ccfMat[[3]],output=HC_folder,distance="correlation")
